...
fn find_sdk() -> PathBuf {
    if let Ok(d) = std::env::var("FIDELITYFX_SDK_DIR") {
        ...
        if p.join("sdk/CMakeLists.txt").exists() {
            return p.join("sdk");
        }
    ...
    panic!(
        "Clone: git clone --depth 1 --branch v1.1.4 https://github.com/GPUOpen-LibrariesAndSDKs/FidelityFX-SDK.git {}/FidelityFX-SDK",
        root.display()
    );
}
...
fn main() {
    ...
    if !types_out.exists() {
        ...
        std::fs::write(
            &types_out,
            s.replace("#pragma warning(", "// #pragma warning(")
                .replace(
